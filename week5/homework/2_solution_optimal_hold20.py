...
def pig_actions_d(state):
    ...
    # Notice how Professor Norvig uses a single assignment statement here,
    # which is nicer to read
    # i.e. as opposed to multiple assignment statements in different
    # if/else blocks
...
# Probability of winning from the state, ignoring doubling
@memo
def win2_p(me, you, pending, is_opt = True):
    if (me + pending >= goal):
        return 1.
    elif (you >= goal):
        return 0.
    roll_p = win2_p_roll(me, you, pending, is_opt)
    if (pending > 0):
        if is_opt:
            return max(1. - win2_p(you, me+pending, 0, not is_opt), roll_p)
        else:
            # This is the hold 20 player, who doesn't play optimally
            if (pending >= 20):
                # Hold
                return 1. - win2_p(you, me+pending, 0, not is_opt)
            else:
                # Roll
                return roll_p
    return roll_p
...
@memo
def win2_p_roll(me, you, pending, is_opt = True):
    return (1.0 - win2_p(you, me+1, 0, not is_opt) + 
            sum([win2_p(me, you, pending+i, is_opt) for i in range(2,7)]))/6.

# Probability of winning given the action
def win2_action_p(state, action):
    (p, me, you, pending, double) = state
    if action == 'accept':
        return win2_p(me, you, pending)
    if action == 'decline':
        return 1. - win2_p(me, you, pending)
    if action == 'double':
        if (win2_p(me, you, pending) > .95):
            return 1.
        else:
            return 0.
    if action == 'hold':
        return 1. - win2_p(you, me + pending, 0, False)
    if action == 'roll':
        return win2_p_roll(me, you, pending)
    raise ValueError

# Return the best action
def strategy_d(state):
    (p, me, you, pending, double) = state 
    legal_actions = pig_actions_d(state)
    def win2_action_p_wrapper(action):
        return win2_action_p(state, action)
    x = max(legal_actions, key=win2_action_p_wrapper)
    # print "{0} {1}".format(x, state)
    return x
