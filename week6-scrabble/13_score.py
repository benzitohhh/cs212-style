...
def calculate_score(board, pos, direction, hand, word):
    ...
    for (n, L) in enumerate(word):
        ...
        # Once again... notice single assignment done in one go - nice one Prof Norvig!
        ...
        # NOTE:
        # cross_word_score recursively calls calculate_score.
        # To prevent infinite loops, the "is not DOWN" clause is used.
        
        # NOTE 2:
        # This is bad, as it assumes that ACROSS will always be called
        # before DOWN. Probably should refactor later..
        # .... although since calculate_score is only called from horizontal_plays,
        # we can guarentee it will always be called with ACROSS first....
    return crosstotal + word_mult * total
    
def cross_word_score(board, L, pos, direction):
    """Return the score of a word in the other direction from the main word."""
    i, j = pos
    (j2, word) = find_cross_word(board, i, j)
    return calculate_score(board, pos, direction, hand, word)