...
def anagrams(phrase, shortest=2):
    ...
    for word in find_words(letters):
        if len(word) < shortest:
            continue
        rem_letters = removed(letters, word)
        if rem_letters == '':
            results.add(word)
        else:
            results |= set([" ".join(sorted((word + " " + phrase).split())) for phrase in anagrams(rem_letters, shortest)])
    return results
...
# WORDS, PREFIXES = readwordlist('words_short.txt')
