...
def poly(coefs):
    ...
    f.__name__ =  " + ".join(([termString(p, c) for p,c in enumerate(coefs) if c!=0])[::-1])
...
def is_poly(x):
    ...
    return hasattr(x, '__call__') and hasattr(x, 'func_doc') and x.func_doc == "Polynomial"

def add(p1, p2):
    "Return a new polynomial which is the sum of polynomials p1 and p2."
    c1, c2 = getPaddedCoefficients(p1, p2)
    coefs = tuple([a + b for a, b in zip(c1, c2)])
    return poly(coefs)
...
def mul(p1, p2):
    ...
    coefs = (len(p1.coefs) + len(p2.coefs) -1) * [0]
    for i, vi in enumerate(p1.coefs):
        for j, vj in enumerate(p2.coefs):
            coefs[i+j] += vi * vj
...
def max_coefficient_length(p1, p2):
    """Returns max num of coefficients"""
    return max(len(p1.coefs), len(p2.coefs))

def pad(t, n):
    """pad tuple to ensure it has length n"""
    return t + (n - len(t)) * (0,) 

def getPaddedCoefficients(p1, p2):
    l = max_coefficient_length(p1, p2)
    return [pad(p1.coefs, l), pad(p2.coefs, l)]
...
def deriv(p):
    ...
    coefs = tuple([(i+1)*v for i,v in enumerate(p.coefs[1:])])
    return poly(coefs)

def integral(p, C=0):
    "Return the integral of a function p (with respect to its argument)."
    coefs = tuple([C,] + [v/(i+1.) for i,v in enumerate(p.coefs)])
    return poly(coefs)
